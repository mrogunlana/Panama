...
namespace Panama.Canal.Comparers
{
    public class DescriptorComparer : IEqualityComparer<IDescriptor>
    {
        ...
        public DescriptorComparer(ILogger log)
        {
            _log = log;
        }

        public bool Equals(IDescriptor? _this, IDescriptor? _that)
        {
            ...
            if (result)
                _log.LogWarning(
                    $"There are duplicate descriptors ({_this!.Topic}) in same group ({_this.Group}) and broker ({_this?.Target?.GetType()?.Name}), this may cause unintended results.");
